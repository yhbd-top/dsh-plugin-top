...
- 点开大面板（最大 880×700，双栏列表）：本地即时搜索 3700+ 插件（仓库名 / 关键词 / 分类，秒响应）
- 分类 chips 按数量排序，可与搜索叠加过滤；点分类看全量，不截断
- **站点同款五个榜**：原生星榜（★排序，全量）· 近期飙升（▲delta）· 今日新秀（+N）· 兼容工具榜（非 DSH 原生生态）· 分类冠军（每分类 ★ 最高，🏆）
